...
<img src="fibonacci.png" width="400">
...
Program Deret Fibonacci Multithread
    ...
    int main(int argc, char *argv[]) {
        if (argc != 2) {
            printf("Penggunaan: %s <jumlah_angka_fibonacci>\n", argv[0]);
            return 1;
        }
...
- Thread anak menghitung deret Fibonacci dan menyimpan di array.

- pthread_join memastikan parent thread menunggu hasil.

- Kompilasi: gcc fibonacci.c -o fibonacci -lpthread
...
Program Manajer PID Multithread
    ...
    void *thread_func(void *arg) {
        ...
        if (pid == -1) {
            printf("PID tidak tersedia!\n");
            pthread_exit(NULL);
        }
    
        printf("Thread %ld: Dapat PID %d\n", pthread_self(), pid);
        sleep(rand() % 5 + 1); // Sleep 1-5 detik
        printf("Thread %ld: Lepas PID %d\n", pthread_self(), pid);
        release_pid(pid);
...
- allocate_pid() dan release_pid() menggunakan mutex untuk thread safety.

- Setiap thread mensimulasikan proses dengan sleep() acak.

- Kompilasi: gcc pid_manager.c -o pid_manager -lpthread

---

• 4.29

Program Echo Server Multithread (Java)
        ...
        public void run() {
            try (
                ...
                while ((inputLine = in.readLine()) != null) {
                    System.out.println("Terima dari klien: " + inputLine);
                    out.println(inputLine); // Echo balik
                }
            } catch (IOException e) {
...
- Setiap koneksi klien ditangani oleh thread terpisah (ClientHandler).

- Server tetap menerima koneksi baru saat menangani klien lain.
